...
/**
 * Deploys a verticle that keeps a registry of services and their latest status.
 * It also regularly polls those services and update their status.
 */
public class MainVerticle extends AbstractVerticle {
  ...
  /**
   * Handles the call to GET /service api. Returns the list of services in the
   * registry.
   */
  private void handleGetServices(RoutingContext routingContext) {
    ...
    registry.getServices().setHandler(res -> {
      if (res.succeeded()) {
        final List<JsonObject> jsonServices = res.result().stream()
            .map(service -> new JsonObject().put("name", service.getName()).put("status", Service.toString(service)))
  ...
  /**
   * Handles the call to POST /service api Add a service to the registry given its
   * name and url. Calling the service twice for the same service will fail.
   */
  private void handlePostService(RoutingContext routingContext) {
    final JsonObject jsonBody = routingContext.getBodyAsJson();
    try {
      registry.addService(jsonBody.getString("name"), jsonBody.getString("url"))
          .setHandler(res -> handleResponse(res, routingContext));
    } catch (IllegalArgumentException e) {
      routingContext.response().putHeader("content-type", "application/json").setStatusMessage(e.toString())
  ...
  private void handleUpdateService(RoutingContext routingContext) {
    final JsonObject jsonBody = routingContext.getBodyAsJson();
    try {
      registry.updateService(jsonBody.getString("name"), jsonBody.getString("url"))
          .setHandler(res -> handleResponse(res, routingContext));
    } catch (IllegalArgumentException e) {
      routingContext.response().putHeader("content-type", "application/json").setStatusMessage(e.toString())
          .setStatusCode(400).end();
  ...
  /**
   * Handles the call to DELETE /service api Removes a service from the registry.
   * Will do nothing if the service is not in the registry.
   */
  private void handleDeleteService(RoutingContext routingContext) {
    final JsonObject jsonBody = routingContext.getBodyAsJson();
    try {
      registry.removeService(jsonBody.getString("name")).setHandler(res -> handleResponse(res, routingContext));
    } catch (IllegalArgumentException e) {
      routingContext.response().putHeader("content-type", "application/json").setStatusMessage(e.toString())
          .setStatusCode(400).end();
    }
  ...
  private void handleResponse(AsyncResult res, RoutingContext routingContext) {
    if (res.succeeded()) {
      ...
      routingContext.response().putHeader("content-type", "application/json").setStatusCode(500)
          .setStatusMessage(res.cause().toString()).end();
    }
  }
}
